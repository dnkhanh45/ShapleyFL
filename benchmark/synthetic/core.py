...
class TaskGen(BasicTaskGen):
    ...
    def run(self):
        ...
        x_trains = [di[:int(5 / 7 * len(di))] for di in xs]
        y_trains = [di[:int(5 / 7 * len(di))] for di in ys]
        x_valids = [di[int(5 / 7 * len(di)):int(6 / 7 * len(di))] for di in xs]
        y_valids = [di[int(5 / 7 * len(di)):int(6 / 7 * len(di))] for di in ys]
        x_tests = [di[int(6 / 7 * len(di)):] for di in xs]
        y_tests = [di[int(6 / 7 * len(di)):] for di in ys]
